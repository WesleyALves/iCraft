...
public class VoiceConnection extends Thread
{
	...
	public boolean canListen(int callCode)
	{
		for(ItemStack itemStack : getPlayer().inventory.mainInventory)
		{
			if(itemStack != null)
			{
				...
				{
					if (itemStack.stackTagCompound.hasKey("called") && itemStack.stackTagCompound.getInteger("called") == 2)
					{
	...
	public int getCallCode()
	{
		for (ItemStack itemStack : getPlayer().inventory.mainInventory)
		{
			...
			{
				if (itemStack.stackTagCompound.hasKey("number"))
				{
					if (itemStack.stackTagCompound.hasKey("called") && itemStack.stackTagCompound.getInteger("called") == 2)
					{
						if (itemStack.stackTagCompound.hasKey("callCode"))
						{
